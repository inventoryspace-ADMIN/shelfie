...
//
// `children` is the dashboard's own item grid, passed in rather than
// rendered by the parent page directly, so this component can hide it
// while the preview is open — otherwise the same items render twice,
// stacked, once for real and once inside the iframe.
export function DevicePreview({
  ...
  useEffect(() => {
    if (!visible || device !== "desktop" || !containerRef.current) return;
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        {visible && (
          ...
          >
            ...
            {device === "mobile" ? (
              // A real device-width viewport, not a squeezed-down layout:
              // an iframe has its own viewport for CSS purposes, so the
              // page's actual mobile breakpoints apply here exactly as
              // they would on an actual phone.
              <div
