...
export async function unpublishSpace(
  spaceId: string
): Promise<{ error: string } | undefined> {
  const supabase = await createClient();
  const {
...
// The one entrypoint that makes a space go live — always carries the slug
// it should publish under, whether that's unchanged (a routine republish)
// or just-edited (the first-publish confirmation dialog in PublishToggle).
// See docs/SCHEMA.md spaces.first_published_at for why that column is set
// here rather than in a migration default.
export async function publishSpace(
  input: PublishSpaceInput
): Promise<{ error: string } | undefined> {
  const parsed = publishSpaceSchema.safeParse(input);
  if (!parsed.success) {
    return { error: parsed.error.issues[0]?.message ?? "Invalid input" };
  }
  const { spaceId, slug } = parsed.data;
  ...
  const { data: space } = await supabase
    .from("spaces")
    .select("owner_id, slug, first_published_at")
    .eq("id", spaceId)
    .single();
  ...
  const { error } = await supabase
    ...
    .update({
      slug,
      status: "published",
      first_published_at: space.first_published_at ?? new Date().toISOString(),
    })
    .eq("id", spaceId);
